...
sub get_tuple {
  ...
  my $query = qq|SELECT ct.*, b.description AS business, s.*, cp.*
                 FROM adressen ct
  ...
  $query = qq|SELECT a.id
              FROM $arap a
	      JOIN adressen ct ON (a.$form->{db}_id = ct.id)
	      WHERE ct.id = $form->{id}
	    UNION
	      SELECT a.id
	      FROM oe a
...
##/LINET
  ...
  if ($form->{id}) {
    ...
    $query = qq|INSERT INTO adressen (name, geschaeftstyp)
                VALUES ('$uid', '$geschaeftstyp')|;
    $dbh->do($query) || $form->dberror($query);
    
    $query = qq|SELECT c.id FROM adressen c
                WHERE c.name = '$uid'|;
    $sth = $dbh->prepare($query);
    $sth->execute || $form->dberror($query);
  ...
  $query = qq|UPDATE adressen SET
	      ...
	      business_id = (SELECT DISTINCT b.id FROM business b
	                     WHERE b.description = '$form->{business}'),
  ...
  } elsif ($form->{cp_name} || $form->{cp_givenname} || $form->{cp_email} || $form->{cp_phone1} || $form->{cp_phone2}) {
...
##/LINET
  ...
  if ($form->{id}) {
    ...
    $query = qq|INSERT INTO adressen (name)
                VALUES ('$uid')|;
    $dbh->do($query) || $form->dberror($query);
   
    $query = qq|SELECT v.id FROM adressen v
                WHERE v.name = '$uid'|;
    $sth = $dbh->prepare($query);
    $sth->execute || $form->dberror($query);
