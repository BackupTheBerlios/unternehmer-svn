...
sub kne_export {
  ...
  if ($form->{exporttype}==0) {
    &kne_buchungsexport($myconfig, $form);
  } else {&kne_stammdatenexport($myconfig, $form);}
...
sub get_transactions {
  ...
  $query = qq|SELECT ac.oid, ac.transdate, ac.trans_id,ar.id, ac.amount, ac.taxkey, ar.invnumber, ar.duedate, ar.amount as umsatz,
              ct.name, c.accno, c.taxkey_id as charttax, c.datevautomatik, c.id, t.chart_id FROM acc_trans ac,ar ar, customer ct,
              ...
              ct.name, c.accno, c.taxkey_id as charttax, c.datevautomatik, c.id, t.chart_id FROM acc_trans ac, ap ap, vendor ct, chart c LEFT JOIN tax t ON
                 (t.chart_id=c.id)
              ...
              gl.description AS name, c.accno,  c.taxkey_id as charttax, c.datevautomatik, c.id, t.chart_id FROM acc_trans ac, gl gl,
              chart c LEFT JOIN tax t ON
