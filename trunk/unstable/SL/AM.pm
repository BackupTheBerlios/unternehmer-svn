...
sub get_business {
  ...
  my $query = qq|SELECT b.description, b.discount
                 FROM business b
	         WHERE b.id = $form->{id}|;
  my $sth = $dbh->prepare($query);
  $sth->execute || $form->dberror($query);
...
sub save_business {
  ...
  if ($form->{id}) {
    ...
    $query = qq|INSERT INTO business 
                ...
                VALUES ('$form->{description}', $form->{discount})|;
  }
  $dbh->do($query) || $form->dberror($query);
...
sub backup {
  ...
  if ($form->{media} eq 'email') {
    ...
    $mail->{subject} = "Unternehmer Backup / $myconfig->{dbname}-$form->{dbversion}.sql";
  ...
  open(FH, "sql/unternehmer.sql") or $form->error("sql/unternehmer.sql : $!");
  ...
  print OUT qq|-- Unternehmer Backup
