...
|;
    ...
    print qq|
	  ...
	  <td colspan=$colspan>
          ...
          <b>$runningnumber</b>&nbsp;<input name="runningnumber_$i" size=3 value=$i>
          <b>$partsgroup</b>&nbsp;<input name="partsgroup_$i" size=10 value="$form->{"partsgroup_$i"}">
          <b>$deliverydate</b>&nbsp;<input name="${delvar}_$i" size=11 title="$myconfig{dateformat}" value="$form->{"${delvar}_$i"}">
|;
...
# $locale->text('Quotation Date missing!')
  ...
  $form->{"bcc"} = $saved_bcc if ($saved_bc);
  ...
  push @a, qw(shiptoname shiptostreet shiptozipcode shiptocity shiptocountry shiptoemail shippingpoint shipvia company address signature employee);
